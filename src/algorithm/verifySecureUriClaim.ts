import { SecureUriVerificationError } from "../error";

/**
 * Verifies a URI claim is secured over TLS
 *
 * Currently restricts to HTTPS URIs.
 ...
 */
export function verifySecureUriClaim(uri: string): void {
  if (!uri.startsWith("https://") && !uri.startsWith("http://localhost:")) {
    throw new SecureUriVerificationError(uri);
  }
}
